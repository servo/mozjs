  ...
  /* check that PR_DuplicateEnvironment() agrees with PR_GetEnv() */
  {
    ...
    if (NULL == dupenv) {
      if (expect_failure) {
        if (verbose)
          printf(
              "env: PR_DuplicateEnvironment failed, "
      ...
      if (expect_failure) {
        if (debug)
          printf(
              "env: PR_DuplicateEnvironment() succeeded, "
      ...
      for (i = dupenv; *i; i++) {
        ...
        if (equals == NULL) {
          if (debug)
            printf(
                "env: PR_DuplicateEnvironment() returned a string"
                " with no '=': %s\n",
                *i);
          failedAlready = PR_TRUE;
        } else {
          ...
          *equals = '\0';
          ...
          if (value == NULL) {
            if (debug)
              printf(
                  "env: PR_DuplicateEnvironment() returned a name"
            ...
            if (debug)
              printf(
                  "env: PR_DuplicateEnvironment() returned the wrong"
            ...
            if (verbose)
              printf(
                  "env: PR_DuplicateEnvironment() agreed with"
                  " PR_GetEnv() about %s\n",
                  *i);
          }
        }
