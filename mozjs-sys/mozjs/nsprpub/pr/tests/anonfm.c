...
static void Help(void) {
  ...
  printf(
      "dirName is alternate directory name. Default: . (current directory)\n");
  exit(1);
} /* end Help() */
...
static void ClientOne(void) {
  ...
  PR_LOG(lm, msgLevel, ("ClientOne() starting"));

  fmString = PR_GetEnv(fmEnvName);
  if (NULL == fmString) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("ClientOne(): PR_Getenv() failed"));
    return;
  }
  PR_LOG(lm, msgLevel, ("ClientOne(): PR_Getenv(): found: %s", fmString));

  fm = PR_ImportFileMapFromString(fmString);
  if (NULL == fm) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("ClientOne(): PR_ImportFileMapFromString() failed"));
    return;
  }
  PR_LOG(lm, msgLevel,
  ...
  PR_LOG(lm, msgLevel, ("ClientOne(): PR_MemMap(): addr: %p", addr));
  ...
  *addr = 1;
  ...
  PR_LOG(lm, msgLevel, ("ClientOne(): PR_MemUnap(): success"));
  ...
  PR_LOG(lm, msgLevel, ("ClientOne(): PR_CloseFileMap(): success"));
...
** ClientTwo() --
*/
static void ClientTwo(void) { failed_already = 1; } /* end ClientTwo() */
...
static void ServerOne(void) {
  ...
  PR_LOG(lm, msgLevel, ("ServerOne() starting"));

  fm = PR_OpenAnonFileMap(dirName, fmSize, fmProt);
  if (NULL == fm) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("PR_OpenAnonFileMap() failed"));
    return;
  }
  PR_LOG(lm, msgLevel, ("ServerOne(): FileMap: %p", fm));

  rc = PR_ExportFileMapAsString(fm, sizeof(fmString), fmString);
  if (PR_FAILURE == rc) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("PR_ExportFileMap() failed"));
    return;
  }
  ...
  */
  ...
  if (NULL == addr) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("PR_MemMap() failed"));
    return;
  }
  ...
  PR_LOG(lm, msgLevel, ("ServerOne(): PR_MemMap(): addr: %p", addr));
  ...
  */
  ...
  PR_ASSERT(proc);
  PR_LOG(lm, msgLevel, ("ServerOne(): PR_CreateProcess(): proc: %x", proc));

  /*
  ** ClientOne() will set the memory to 1
  */
  PR_LOG(lm, msgLevel, ("ServerOne(): waiting on Client, *addr: %x", *addr));
  while (*addr == 0x00) {
    if (debug) {
      fprintf(stderr, ".");
    }
  ...
  if (PR_FAILURE == rc) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("PR_MemUnmap() failed"));
    return;
  }
  PR_LOG(lm, msgLevel, ("ServerOne(): PR_MemUnmap(): success"));

  rc = PR_CloseFileMap(fm);
  if (PR_FAILURE == rc) {
    failed_already = 1;
    PR_LOG(lm, msgLevel, ("PR_CloseFileMap() failed"));
    return;
  }
  PR_LOG(lm, msgLevel, ("ServerOne(): PR_CloseFileMap() success"));
...
static void ServerTwo(void) {
  PR_LOG(lm, msgLevel, ("ServerTwo(): Not implemented yet"));
} /* end ServerTwo() */
