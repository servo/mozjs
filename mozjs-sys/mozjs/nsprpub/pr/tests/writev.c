...
int PR_CALLBACK Writev(int argc, char** argv) {
  ...
  number_fragments =
      (message_length + fragment_length - 1) / fragment_length + 1;
  while (IOV_MAX < number_fragments) {
    fragment_length = message_length / (IOV_MAX - 2);
    number_fragments =
        (message_length + fragment_length - 1) / fragment_length + 1;
    if (NULL != debug)
      PR_fprintf(debug, "Too many fragments - reset fragment length to %ld\n",
                 fragment_length);
  }
  ...
  for (loop = 0; loop < messages; ++loop) {
    ...
    if (clientSock) {
      ...
      if (PR_SUCCESS == rv) {
        ...
        if (NULL != debug)
          PR_fprintf(debug, "sending %d bytes ... ", descriptor.length);
        ...
        if (NULL != debug)
          PR_fprintf(debug, "failed - retrying (%d, %d)\n", PR_GetError(),
                     PR_GetOSError());
        PR_Close(clientSock);
      }
    } else if (NULL != debug) {
...
int main(int argc, char** argv) {
  return (PR_VersionCheck(PR_VERSION)) ? PR_Initialize(Writev, argc, argv, 4)
                                       : -1;
} /* main */
