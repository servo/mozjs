...
static void testParseTimeString(PRTime t) {
  PRExplodedTime et;
  PRTime t2;
  char timeString[128];
  char buf[128];
  ...
  PR_snprintf(timeString, 128, "%s %s %ld %02ld:%02ld:%02ld ",
              dayOfWeek[et.tm_wday], month[et.tm_month], et.tm_mday, et.tm_hour,
              et.tm_min, et.tm_sec);
  /* Print time zone */
  totalOffset = et.tm_params.tp_gmt_offset + et.tm_params.tp_dst_offset;
  if (totalOffset == 0) {
  ...
  */
...

  {
    ...
    if (!ExplodedTimeIsEqual(&et1, &et2)) {
      if (debug_mode) {
        printf(
            "ERROR: PR_ImplodeTime and PR_ExplodeTime are not inverse for "
            "April 4, 1917 GMT\n");
      }
      failed_already = 1;
      return 1;
    ...
    if (!ExplodedTimeIsEqual(&et1, &et2)) {
      if (debug_mode) {
        printf(
            "ERROR: PR_ImplodeTime and PR_ExplodeTime are not inverse for July "
            "4, 2050 GMT\n");
      }
      failed_already = 1;
      return 1;
      ...
      for (hour = 0; hour < 24; hour++) {
        for (min = 0; min < 60; min += 10) {
          ...
          if (LL_NE(usecs, PR_ImplodeTime(&et1))) {
            printf(
                "ERROR: PR_ExplodeTime and PR_ImplodeTime are not inverse\n");
            PrintExplodedTime(&et1);
            printf("\n");
            failed_already = 1;
            return 1;
