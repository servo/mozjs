...
static PRStatus PrintAddress(const PRNetAddr* address) {
  ...
  if (PR_FAILURE == rv) {
    ...
    PR_fprintf(std_out, "Accepted connection from (0x%p)%s:%d\n", address,
...
static void AcceptingThread(void* arg) {
  ...
  if (-1 == bytes) {
    ...
    PR_fprintf(std_out, "(Server) read [0x%p..0x%p) %s\n", buf, &buf[BUF_SIZE],
               buf);
...
int main(int argc, char** argv) {
  ...
  if (argc < 3) {
    ...
    status = PR_GetHostByName(argv[1], netdb_buf, sizeof(netdb_buf), &he);
  ...
  for (write_dally = 0; write_dally < accept_timeout + (2 * delta);
       write_dally += delta) {
    PR_fprintf(std_out, "Testing w/ write_dally = %d msec\n",
