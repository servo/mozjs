...
static void serve_client_read(void* arg) {
  ...
  if (sp->bytes_read < sp->bytes) {
    jobp = PR_QueueJob_Read(sp->tp, &sp->iod, serve_client_read, sp, PR_FALSE);
    PR_ASSERT(NULL != jobp);
    return;
  }
  PR_ASSERT(sp->bytes_read == sp->bytes);
  ...
  sp->iod.timeout = PR_SecondsToInterval(60);
  jobp = PR_QueueJob_Write(sp->tp, &sp->iod, serve_client_write, sp, PR_FALSE);
  PR_ASSERT(NULL != jobp);
...
static void print_stats(void* arg) {
  ...
  jobp = PR_QueueJob_Timer(tp, PR_MillisecondsToInterval(500), print_stats, sp,
                           PR_FALSE);
...
static void PR_CALLBACK TCP_Server(void* arg) {
  ...
  jobp = PR_QueueJob_Accept(tp, &sp->iod, TCP_Server_Accept, sp, PR_FALSE);
  PR_ASSERT(NULL != jobp);
...
#define MAX_ARGS 4
  ...
  jobp = PR_QueueJob_Timer(tp, PR_MillisecondsToInterval(5000), print_stats, sp,
                           PR_FALSE);
  PR_ASSERT(NULL != jobp);
  rval = PR_CancelJob(jobp);
  PR_ASSERT(PR_SUCCESS == rval);
...
static void TCP_Server_Accept(void* arg) {
   ...
   */
  sp->conn_counter++;
  if (sp->conn_counter < (num_tcp_clients * num_tcp_connections_per_client)) {
    jobp = PR_QueueJob_Accept(tp, &sp->iod, TCP_Server_Accept, sp, PR_FALSE);
    PR_ASSERT(NULL != jobp);
    return;
  }
