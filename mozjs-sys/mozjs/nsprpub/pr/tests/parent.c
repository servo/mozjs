...
int main(int argc, char** argv) {
  ...
  PR_ProcessAttrSetStdioRedirect(child->attr, PR_StandardOutput,
                                 PR_GetSpecialFD(PR_StandardOutput));
  PR_ProcessAttrSetStdioRedirect(child->attr, PR_StandardError,
                                 PR_GetSpecialFD(PR_StandardError));

  t_start = PR_IntervalNow();
  child->process =
      PR_CreateProcess(child->name, child->argv, NULL, child->attr);
  ...
  if (NULL != debug) {
    PR_fprintf(debug, "Child was %sforked\n", (0 == test_status) ? "" : "NOT ");
    if (0 == test_status)
      PR_fprintf(debug, "PR_CreateProcess took %lu microseconds\n",
  ...
  if (0 == test_status) {
    ...
    if (PR_SUCCESS == rv) {
      if (NULL != debug)
        PR_fprintf(debug, "Child exited %s\n",
