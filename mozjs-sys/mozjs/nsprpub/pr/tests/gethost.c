...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    if (PR_GetHostByAddr(&addr, reversebuf, sizeof(reversebuf), &reversehe) ==
        PR_FAILURE) {
  ...
  if (PR_GetIPNodeByName(hostName, PR_AF_INET, PR_AI_DEFAULT, buf, sizeof(buf),
                         &he) == PR_FAILURE) {
  ...
  if (PR_GetIPNodeByName(hostName, PR_AF_INET6, PR_AI_DEFAULT, buf, sizeof(buf),
                         &he) == PR_FAILURE) {
    fprintf(stderr, "PR_GetIPNodeByName failed\n");
    exit(1);
  }
