...
static void PR_CALLBACK AbortCV(void* arg) {
  ...
  if (debug_mode) {
    printf("Expected success on wait CV and %s\n",
...
static void PR_CALLBACK AbortJoin(void* arg) {} /* AbortJoin */
...
void PR_CALLBACK Intrupt(void* arg) {
  ...
  abortJoin = PR_CreateThread(PR_USER_THREAD, AbortJoin, 0, PR_PRIORITY_NORMAL,
                              thread_scope, PR_JOINABLE_THREAD, 0);
  PR_Sleep(PR_SecondsToInterval(2));
  if (debug_mode) {
  ...
  setup_listen_socket(&listner, &netaddr);
  abortIO =
      PR_CreateThread(PR_USER_THREAD, AbortIO, PR_GetCurrentThread(),
  ...
  intrBlock = PR_CreateThread(PR_USER_THREAD, IntrBlock, 0, PR_PRIORITY_NORMAL,
                              thread_scope, PR_JOINABLE_THREAD, 0);
