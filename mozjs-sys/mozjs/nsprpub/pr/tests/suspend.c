...
void PR_CALLBACK Level_1_Thread(void* arg) {
  ...
  thr = PR_CreateThreadGCAble(PR_USER_THREAD, Level_2_Thread, NULL,
                              PR_PRIORITY_HIGH, scope, PR_JOINABLE_THREAD, 0);
...
static void Level_0_Thread(PRThreadScope scope1, PRThreadScope scope2) {
  ...
  alive = count;
  for (n = 0; n < count; n++) {
    thr = PR_CreateThreadGCAble(PR_USER_THREAD, Level_1_Thread, (void*)scope2,
                                PR_PRIORITY_NORMAL, scope1,
