...
**                 The debug mode will print all of the printfs associated with
*this test.
**                         The regress mode will be the default mode. Since the
*regress tool limits
...
** 12-June-97 AGarcic - Revert to return code 0 and 1, remove debug option
*(obsolete).
***********************************************************************/
...
static void ClientThreadFunc(void* arg) {
  ...
  fflush(stdout);
  if (PR_Connect(sock, &addr, PR_SecondsToInterval(5)) != PR_SUCCESS) {
