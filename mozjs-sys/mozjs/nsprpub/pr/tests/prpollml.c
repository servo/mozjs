...
#define POLL_DESC_COUNT                  \
  256 /* This should be greater than the \
...
static void Test(void) {
  ...
  for (i = 1; i <= POLL_DESC_COUNT; i++) {
    ...
    if (rv != 0) {
      fprintf(stderr, "PR_Poll should time out but returns %d (%d, %d)\n",
...
static void ThreadFunc(void* arg) { Test(); }
...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < POLL_DESC_COUNT; i++) {
    ...
    if (sock == NULL) {
      fprintf(stderr, "PR_NewTCPSocket failed (%d, %d)\n", (int)PR_GetError(),
              (int)PR_GetOSError());
      fprintf(stderr,
              "Ensure the per process file descriptor limit "
              "is greater than %d.",
              POLL_DESC_COUNT);
      exit(1);
    }
  ...
  thread = PR_CreateThread(PR_USER_THREAD, ThreadFunc, NULL, PR_PRIORITY_NORMAL,
                           PR_GLOBAL_BOUND_THREAD, PR_JOINABLE_THREAD, 0);
  if (NULL == thread) {
    fprintf(stderr, "PR_CreateThread failed\n");
    exit(1);
  }
