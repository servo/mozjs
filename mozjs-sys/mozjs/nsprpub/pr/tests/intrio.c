...
static void Test(PRThreadScope scope1, PRThreadScope scope2) {
  ...
  iothread = PR_CreateThread(PR_USER_THREAD, IOThread, NULL, PR_PRIORITY_NORMAL,
  ...
  PR_Unlock(lock);
  abortio = PR_CreateThread(PR_USER_THREAD, AbortIO, iothread,
                            PR_PRIORITY_NORMAL, scope2, PR_JOINABLE_THREAD, 0);
  if (abortio == NULL) {
    fprintf(stderr, "cannot create thread\n");
    exit(1);
  }
