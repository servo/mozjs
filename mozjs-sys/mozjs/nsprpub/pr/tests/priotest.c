...
static void Help(void) {
  PR_fprintf(debug_out, "Usage: priotest [-d] [-c n]\n");
...
static void RudimentaryTests(void) {
  ...
  */
  ...
  failed = ((PR_TRUE == failed) || (PR_PRIORITY_URGENT != priority)) ? PR_TRUE
                                                                     : PR_FALSE;
  ...
  PR_SetThreadPriority(PR_GetCurrentThread(),
                       (PRThreadPriority)(PR_PRIORITY_FIRST - 1));
  priority = PR_GetThreadPriority(PR_GetCurrentThread());
  failed = ((PR_TRUE == failed) || (PR_PRIORITY_FIRST != priority)) ? PR_TRUE
                                                                    : PR_FALSE;
  ...
  PR_SetThreadPriority(PR_GetCurrentThread(),
                       (PRThreadPriority)(PR_PRIORITY_LAST + 1));
  priority = PR_GetThreadPriority(PR_GetCurrentThread());
  failed = ((PR_TRUE == failed) || (PR_PRIORITY_LAST != priority)) ? PR_TRUE
                                                                   : PR_FALSE;
...
static void CreateThreads(PRUint32* lowCount, PRUint32* highCount) {
  (void)PR_CreateThread(PR_USER_THREAD, Low, lowCount, PR_PRIORITY_LOW,
                        PR_LOCAL_THREAD, PR_JOINABLE_THREAD, 0);
  (void)PR_CreateThread(PR_USER_THREAD, High, highCount, PR_PRIORITY_HIGH,
                        PR_LOCAL_THREAD, PR_JOINABLE_THREAD, 0);
} /* CreateThreads */
