...
# taken from https://github.com/servo/servo/blob/1728d63f4886e701f3e8236a0384461ed8880b13/etc/ci/publish_crates_io.py
...
USER_AGENT = "servo-publish-crates-io/1 (https://github.com/servo/mozjs)"

WORKSPACE_ROOT = Path(__file__).resolve().parents[0]
SLEEP_AFTER_PUBLISH_SECONDS = int(
    os.environ.get("SERVO_CRATES_IO_SLEEP_AFTER_PUBLISH_SECONDS", "30")
)
VERIFY_PUBLISHED_TIMEOUT_SECONDS = int(
    os.environ.get("SERVO_CRATES_IO_VERIFY_PUBLISHED_TIMEOUT_SECONDS", "300")
)
VERIFY_PUBLISHED_INTERVAL_SECONDS = int(
    os.environ.get("SERVO_CRATES_IO_VERIFY_PUBLISHED_INTERVAL_SECONDS", "10")
)
...
MAX_RUNTIME_SECONDS = int(
    os.environ.get("SERVO_CRATES_IO_MAX_RUNTIME_SECONDS", str(25 * 60))
)
...
def collect_packages(metadata: dict) -> dict[str, WorkspacePackage]:
    ...
    publishable_names = {
        name
        for name, package in local_packages.items()
        if publishes_to_crates_io(package)
    }
    ...
    for name in sorted(publishable_names):
        ...
        if blocked_dependencies:
            ...
            raise ValueError(
                f"{name} depends on local crate(s) that do not publish to crates.io: {blocked}"
            )
...
def crates_io_version_exists(crate_name: str, version: str) -> bool:
    ...
    try:
        ...
        raise RuntimeError(
            f"failed to query crates.io for {crate_name} {version}: {url_error.reason}"
        ) from url_error
...
def wait_until_published(package: WorkspacePackage) -> None:
    ...
    while True:
        ...
        if remaining <= 0:
            raise RuntimeError(
                f"timed out waiting for {package.name} {package.version} to appear on crates.io"
            )
...
def publish_packages(
    args: argparse.Namespace, packages: Iterable[WorkspacePackage], start_time: float
) -> bool:
    for package in packages:
        if runtime_limit_reached(start_time):
            log(
                f"reached runtime limit of {MAX_RUNTIME_SECONDS}s; stopping before publishing {package.name}"
            )
            return True
