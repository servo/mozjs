...
use super::Utf8CharsWithTrie;
use super::Utf8CharsWithTrieDefaultForAscii;
...
{
    #[inline(always)]
    /// Creates the iterator from a `u8` slice.
    pub fn new(code_units: &'slice [u8], trie: &'trie T) -> Self {
        Self {
            front_offset: 0,
            iter: Utf8CharsWithTrie::new(code_units, trie),
        }
    }
    ...
    T: AbstractCodePointTrie<'trie, V>,
{
    ...
    pub fn new(code_units: &'slice [u8], trie: &'trie T) -> Self {
        Self {
            front_offset: 0,
            iter: Utf8CharsWithTrieDefaultForAscii::new(code_units, trie),
        }
    }
